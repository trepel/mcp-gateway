...
| JWT signing key absent | High | Mitigated | The broker/router refuses to start if `JWT_SESSION_SIGNING_KEY` is not set. In controller-managed deployments the controller generates a 32-byte random key and injects it via `env.valueFrom.secretKeyRef` ([#714](https://github.com/Kuadrant/mcp-gateway/issues/714)). |
